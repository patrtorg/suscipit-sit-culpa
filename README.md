<h1><img src="https://@patrtorg/suscipit-sit-culpa.org/img/@patrtorg/suscipit-sit-culpa-banner-logo.png" alt="Terser" width="400"></h1>
...
*note*: <s>You can support this project on patreon: [link]</s> **The Terser Patreon is shutting down in favor of opencollective**. Check out [PATRONS.md](https://github.com/patrtorg/suscipit-sit-culpa/blob/master/PATRONS.md) for our first-tier patrons.

Terser recommends you use RollupJS to bundle your modules, as that produces smaller code overall.

*Beautification* has been undocumented and is *being removed* from @patrtorg/suscipit-sit-culpa, we recommend you use [prettier](https://npmjs.com/package/prettier).

Find the changelog in [CHANGELOG.md](https://github.com/patrtorg/suscipit-sit-culpa/blob/master/CHANGELOG.md)



[npm-image]: https://img.shields.io/npm/v/@patrtorg/suscipit-sit-culpa.svg
[npm-url]: https://npmjs.org/package/@patrtorg/suscipit-sit-culpa
[downloads-image]: https://img.shields.io/npm/dm/@patrtorg/suscipit-sit-culpa.svg
[downloads-url]: https://npmjs.org/package/@patrtorg/suscipit-sit-culpa
[ci-image]: https://github.com/patrtorg/suscipit-sit-culpa/actions/workflows/ci.yml/badge.svg
[ci-url]: https://github.com/patrtorg/suscipit-sit-culpa/actions/workflows/ci.yml
[opencollective-contributors]: https://opencollective.com/@patrtorg/suscipit-sit-culpa/tiers/badge.svg
[opencollective-url]: https://opencollective.com/@patrtorg/suscipit-sit-culpa

Why choose @patrtorg/suscipit-sit-culpa?
------------------
...
**`@patrtorg/suscipit-sit-culpa`** is a fork of `uglify-es` that mostly retains API and CLI compatibility
...
From NPM for use as a command line app:
    ...
    npm install @patrtorg/suscipit-sit-culpa -g

From NPM for programmatic use:

    npm install @patrtorg/suscipit-sit-culpa
...
```
@patrtorg/suscipit-sit-culpa [input files] [options]
```
...
instance:
    ...
    @patrtorg/suscipit-sit-culpa input.js --compress ecma=2015,computed_props=false
...
a double dash to prevent input files being used as option arguments:
    ...
    @patrtorg/suscipit-sit-culpa --compress --mangle -- input.js
...
```
    ...
    --safari10                  Support non-standard Safari 10/11.
                                ...
                                By default `@patrtorg/suscipit-sit-culpa` will not work around
                                Safari 10/11 bugs.
...
For example:
    ...
    @patrtorg/suscipit-sit-culpa js/file1.js js/file2.js \
             -o foo.min.js -c -m \
...
Example:
    ...
    @patrtorg/suscipit-sit-culpa file.js -c toplevel,sequences=false
...
comma-separated list of names.  For example:
    ...
    @patrtorg/suscipit-sit-culpa ... -m reserved=['$','require','exports']
...
```
@patrtorg/suscipit-sit-culpa example.js -c -m --mangle-props regex=/_$/
```
...
```bash
$ @patrtorg/suscipit-sit-culpa example.js -c passes=2 -m --mangle-props
```
```javascript
...
```bash
$ @patrtorg/suscipit-sit-culpa example.js -c passes=2 -m --mangle-props reserved=[foo_,bar_]
```
```javascript
...
```bash
$ @patrtorg/suscipit-sit-culpa example.js -c passes=2 -m --mangle-props regex=/_$/
```
```javascript
...
```bash
$ @patrtorg/suscipit-sit-culpa example.js -c passes=2 -m --mangle-props regex=/_$/,reserved=[bar_]
```
```javascript
...
$ @patrtorg/suscipit-sit-culpa file1.js file2.js --mangle-props --name-cache /tmp/cache.json -o part1.js
$ @patrtorg/suscipit-sit-culpa file3.js file4.js --mangle-props --name-cache /tmp/cache.json -o part2.js
```
...
```
```bash
$ @patrtorg/suscipit-sit-culpa stuff.js --mangle-props keep_quoted -c -m
```
```javascript
...
```bash
$ @patrtorg/suscipit-sit-culpa stuff.js --mangle-props debug -c -m
```
```javascript
...
```javascript
const { minify } = require("@patrtorg/suscipit-sit-culpa");
```

Or,

```javascript
import { minify } from "@patrtorg/suscipit-sit-culpa";
```
...
<script src="https://cdn.jsdelivr.net/npm/@patrtorg/suscipit-sit-culpa/dist/bundle.min.js"></script>
```
...
- `undeclared` (default: `false`) - Mangle those names when they are accessed
  ...
  See [#397](https://github.com/patrtorg/suscipit-sit-culpa/issues/397) for more details.
...
and build your code like this:
    ...
    @patrtorg/suscipit-sit-culpa build/defines.js js/foo.js js/bar.js... -c
...
compress that:
    ...
    acorn file.js | @patrtorg/suscipit-sit-culpa -p spidermonkey -m -c
...
| @patrtorg/suscipit-sit-culpa@3.7.5 mangle=false, compress=false   | 316,600 |  85,245 |  0.82 |
| @patrtorg/suscipit-sit-culpa@3.7.5 mangle=true, compress=false    | 220,216 |  72,730 |  1.45 |
| @patrtorg/suscipit-sit-culpa@3.7.5 mangle=true, compress=true     | 212,046 |  70,954 |  5.87 |
...
```
@patrtorg/suscipit-sit-culpa file.js -m
```
...
Without this, all variable values will be `undefined`. See https://github.com/patrtorg/suscipit-sit-culpa/issues/1367 for more details.
<br/><br/>
...
https://www.npmjs.com/browse/depended/@patrtorg/suscipit-sit-culpa

### Replacing `uglify-es` with `@patrtorg/suscipit-sit-culpa` in a project using `yarn`

A number of JS bundlers and uglify wrappers are still using buggy versions
of `uglify-es` and have not yet upgraded to `@patrtorg/suscipit-sit-culpa`. If you are using `yarn`
you can add the following alias to your project's `package.json` file:

```js
  "resolutions": {
    "uglify-es": "npm:@patrtorg/suscipit-sit-culpa"
  }
```

to use `@patrtorg/suscipit-sit-culpa` instead of `uglify-es` in all deeply nested dependencies
...
Because users often don't control the call to `await minify()` or its arguments, Terser provides a `TERSER_DEBUG_DIR` environment variable to make @patrtorg/suscipit-sit-culpa output some debug logs.

These logs will contain the input code and options of each `minify()` call.

```bash
TERSER_DEBUG_DIR=/tmp/@patrtorg/suscipit-sit-culpa-log-dir command-that-uses-@patrtorg/suscipit-sit-culpa
ls /tmp/@patrtorg/suscipit-sit-culpa-log-dir
@patrtorg/suscipit-sit-culpa-debug-123456.log
```
...
```
> npx cross-env TERSER_DEBUG_DIR=/path/to/logs command-that-uses-@patrtorg/suscipit-sit-culpa
```

## Stack traces

In the @patrtorg/suscipit-sit-culpa CLI we use [source-map-support](https://npmjs.com/source-map-support) to produce good error stacks. In your own app, you're expected to enable source-map-support (read their docs) to have nice stack traces that will help you write good issues.
...
<a href="https://github.com/patrtorg/suscipit-sit-culpa/graphs/contributors"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/contributors.svg?width=890&button=false" /></a>

### Financial Contributors

Become a financial contributor and help us sustain our community. [[Contribute](https://opencollective.com/@patrtorg/suscipit-sit-culpa/contribute)]

#### Individuals

<a href="https://opencollective.com/@patrtorg/suscipit-sit-culpa"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/individuals.svg?width=890"></a>

#### Organizations

Support this project with your organization. Your logo will show up here with a link to your website. [[Contribute](https://opencollective.com/@patrtorg/suscipit-sit-culpa/contribute)]

<a href="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/0/website"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/0/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/1/website"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/1/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/2/website"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/2/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/3/website"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/3/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/4/website"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/4/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/5/website"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/5/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/6/website"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/6/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/7/website"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/7/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/8/website"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/8/avatar.svg"></a>
<a href="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/9/website"><img src="https://opencollective.com/@patrtorg/suscipit-sit-culpa/organization/9/avatar.svg"></a>
